...
/** Service class for central exception reporting in a Struts 1.3.8 environment.
 ...
 */
@Override
public ActionForward execute(final Exception ex, final ExceptionConfig ae, final ActionMapping mapping, final ActionForm form, final HttpServletRequest request, final HttpServletResponse response) 
throws ServletException 
{
	...
	logger.info("globalErrorForward=" + globalErrorForward);
	
    final String globalReportingForward = globalErrorForward.getPath();
    final ResourceBundle responseBundle = getBundle(BASE_NAME, request);
    final ActionMessages errorsMut = new ActionErrors();
    errorsMut.add( ActionMessages.GLOBAL_MESSAGE, new ActionMessage(
		ae.getKey(),
        getMessagesAsHtml(ex, responseBundle), globalReportingForward
    ));
    serviceAction.mySaveErrors(request, errorsMut);

    final String inputFormPath = mapping.getInput();
    final String reportingFormPath = inputFormPath==null ? globalReportingForward : inputFormPath;
	logger.warning("Now forwarding to reporting form path '" + reportingFormPath + "'");
	final ActionForward inputForward = mapping.getInputForward();
	if(inputForward!=null){return inputForward;}
	return mapping.findForward(globalReportingForward);
}
	 ...
	 */
	public static ResourceBundle getBundle(final String baseName, final HttpServletRequest request){
